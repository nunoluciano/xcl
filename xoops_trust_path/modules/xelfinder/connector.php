...
try {
	//debug($_REQUEST);
	// for keep alive
	if ( ! empty( $_GET['keepalive'] ) && 'OPTIONS' !== $_SERVER['REQUEST_METHOD'] ) {
		exit( 0 );
	}
	...
	$php54 = PHP_VERSION_ID >= 50400;
	$php55 = PHP_VERSION_ID >= 50500;
	...
	if ( empty( $_REQUEST['xoopsUrl'] ) && ! $origin ) {
		...
		if ( ( $origin && ! isset( $allowOrigins[ $origin ] ) )
		     || ( isset( $_SERVER['HTTP_ACCESS_CONTROL_REQUEST_METHOD'] )
		          && ! in_array( strtoupper( $_SERVER['HTTP_ACCESS_CONTROL_REQUEST_METHOD'] ), [
					'POST',
					'GET',
					'OPTIOINS'
				] ) )
		...
		if ( 'OPTIONS' === $_SERVER['REQUEST_METHOD'] || ! empty( $_GET['keepalive'] ) ) {
			exit( 0 );
		}
	}
		...
		if ( defined( 'ELFINDER_DROPBOX_APPKEY' ) && $config['dropbox_path'] && $config['dropbox_acc_token'] ) {
			...
			if ( empty( $config['dropbox_acc_seckey'] ) || $token2 ) {
				...
				$dropbox      = [
					...
					'defaults'      => [
						'read'   => true,
						'write'  => ( $dropboxIsInGroup ? true : false ),
						'hidden' => false,
						'locked' => false
					],
					'accessControl' => is_object( $dropbox_access ) ? [ $dropbox_access, 'access' ] : null,
					'uploadDeny'    => ( ! $isAdmin && ! empty( $config['dropbox_upload_mime'] ) ) ? [ 'all' ] : [],
					'uploadAllow'   => ( ! $isAdmin && ! empty( $config['dropbox_upload_mime'] ) ) ? array_map( 'trim', explode( ',', $config['dropbox_upload_mime'] ) ) : [],
	...
	// check netVolumeData
	if ( $netVolumeData ) {
		if ( count( $netVolumeData ) !== count( $session->get( 'netvolume', [] ) ) ) {
