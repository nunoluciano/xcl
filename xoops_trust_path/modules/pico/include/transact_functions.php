...
function pico_sync_cattree($mydirname)
{
    ...
    [$tree_array, $subcattree, $contents_total, $subcategories_total, $subcategories_ids_cs] = pico_makecattree_recursive($mydirname, 0);
...
function pico_makecattree_recursive($mydirname, $cat_id, $order = 'cat_weight', $parray = [], $depth = 0, $cat_title = '')
{
    ...
    while (list($new_cat_id, $new_cat_title) = $db->fetchRow($result)) {
        [$parray, $subarray, $contents_smallsum, $subcategories_smallsum, $subcateroeis_ids_cs_sub] = pico_makecattree_recursive($mydirname, $new_cat_id, $order, $parray, $depth + 1, $new_cat_title);
...
function pico_transact_reset_body_cached($mydirname, $content_id)
{
    ...
    [$use_cache, $body] = $db->fetchRow($db->query('SELECT use_cache,body FROM ' . $db->prefix($mydirname . '_contents') . " WHERE content_id=$content_id"));
...
function pico_transact_backupcontent($mydirname, $content_id, $forced = false)
{
    ...
    [$last_ch_modified, $last_ch_4search] = $db->fetchRow($db->query('SELECT `modified_time`,MD5(`body`) FROM ' . $db->prefix($mydirname . '_content_histories') . ' WHERE content_history_id=' . (int)$last_ch_id));
    ...
    if ($histories_per_content > 0) {
        do {
            [$ch_count, $min_ch_id] = $db->fetchRow($db->query('SELECT COUNT(*),MIN(content_history_id) FROM ' . $db->prefix($mydirname . '_content_histories') . ' WHERE content_id=' . (int)$content_id));
