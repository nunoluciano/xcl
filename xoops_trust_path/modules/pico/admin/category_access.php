...
[$cat_id, $pid, $cat_title, $redundants_serialized, $cat_permission_id] = $db->fetchRow($db->query('SELECT cat_id,pid,cat_title,cat_redundants,cat_permission_id FROM ' . $db->prefix($mydirname . '_categories') . " WHERE cat_id=$cat_id"));
if (empty($cat_id)) {
    $cat_id = 0;
