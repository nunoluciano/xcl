...
class elFinderVolumeDropbox2 extends elFinderVolumeDriver {
	...
	protected function doSearch( $path, $q, $mimes ) {
		...
		while ( $more ) {
			...
			$searchRes = $this->service->search( $path, $q, [
				'start'       => $searchRes->getCursor(),
				'max_results' => 1000
			] );
			$more      = $searchRes->hasMoreItems();
	...
	public function getContentUrl( $hash, $options = [] ) {
		...
		if ( ( $file = $this->file( $hash ) ) !== false && ( ! $file['url'] || $file['url'] == 1 ) ) {
			...
			$url  = '';
			try {
				$res = $this->service->postToAPI( '/sharing/list_shared_links', [
					'path'        => $path,
					'direct_only' => true
				] )->getDecodedBody();
				...
				if ( ! $url ) {
					$res = $this->service->postToAPI( '/sharing/create_shared_link_with_settings', [
						'path'     => $path,
						'settings' => $this->options['publishPermission']
					] )->getDecodedBody();
					if ( isset( $res['url'] ) ) {
						$url = $res['url'];
					}
