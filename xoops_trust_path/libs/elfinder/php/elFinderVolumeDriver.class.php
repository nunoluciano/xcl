...
abstract class elFinderVolumeDriver {
			...
			if ( $res && $this->options['tmbGcMaxlifeHour'] && $this->options['tmbGcPercentage'] > 0 ) {
				...
				if ( $rand <= $this->options['tmbGcPercentage'] * 100 ) {
					register_shutdown_function( array(
						'elFinder',
						'GlobGC'
					), $this->tmbPath . DIRECTORY_SEPARATOR . '*.png', $this->options['tmbGcMaxlifeHour'] * 3600 );
				}
			}
	...
	public function zipdl( $hashes ) {
		...
		if ( $dir = $this->getItemsInHand( $hashes ) ) {
			...
			register_shutdown_function( array(
				'elFinder',
				'GlobGC'
			), $pdir . DIRECTORY_SEPARATOR . $tmppre . '*', 7200 );
	...
	public function getTempLinkInfo( $name = null ) {
		if ( $this->tmpLinkPath ) {
			...
			register_shutdown_function( array(
				'elFinder',
				'GlobGC'
			), $this->tmpLinkPath . DIRECTORY_SEPARATOR . 'temp_*', elFinder::$tmpLinkLifeTime );
	...
	protected function getItemsInHand( $hashes, $dir = null, $canLink = null ) {
		...
		if ( is_null( $banChrs ) ) {
			$banChrs = DIRECTORY_SEPARATOR !== '/' ? array(
				'\\',
				'/',
				':',
				'*',
	...
	protected function copyFrom( $volume, $src, $destination, $name ) {
		...
		try {
			...
			elFinder::$instance->trigger( 'paste.copyfrom', array(
				&$thash,
				&$name,
				'',
				elFinder::$instance,
	...
	protected function getArchivers( $use_cache = true ) {
		...
		if ( class_exists( 'ZipArchive', false ) ) {
			...
			if ( empty( $arcs['extract']['application/zip'] ) ) {
				$arcs['extract']['application/zip'] = array(
					'cmd'  => 'phpfunction',
					'argc' => array( 'self', 'zipArchiveUnzip' ),
					'ext'  => 'zip'
				);
			}
		}
