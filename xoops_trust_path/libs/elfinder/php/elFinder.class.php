...
class elFinder {
	...
	/**
	 * elFinder common temporary path
	 *
	 * @var string
			...
			if ( $_cmd !== '' ) {
				if ( $all = ( strpos( $_cmd, '*' ) !== false ) ) {
					...
					if ( $sub ) {
						...
						$cmds = array_merge( $cmds, array_map( array(
							'elFinder',
							'addSubToBindName'
						), $allCmds, $subs ) );
					} else {
			...
			if ( $renames ) {
				...
				if ( isset( $renames[ $file['name'] ] ) ) {
					...
					if ( $srcVolume !== $dstVolume ) {
						$errors = array();
						try {
							$this->trigger( 'paste.copyfrom', array(
								&$dst,
								&$dstName,
								'',
								$this,
					...
					$rnres = $this->rename( array(
						'target' => $hash,
						'name'   => $dstVolume->uniqueName( $dir, $dstName, $suffix, true, 0 )
					) );
					if ( ! empty( $rnres['error'] ) ) {
					...
					if ( ! empty( $this->listeners['get.detectencoding'] ) ) {
						foreach ( $this->listeners['get.detectencoding'] as $handler ) {
							call_user_func_array( $handler, array(
								'get',
								&$enc,
								array_merge( $args, array( 'content' => $content ) ),
								$this,
								$volume
							) );
						}
