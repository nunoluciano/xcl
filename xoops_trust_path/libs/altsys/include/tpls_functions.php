    ...
    if ('default' != $tplset) {
        ...
        if ($count <= 0) {
            ...
            while ($row = $db->fetchArray($result)) {
                $db->queryF(
                    'INSERT INTO '
                    . $db->prefix('tplfile') . " SET tpl_refid='" . addslashes($row['tpl_refid']) . "',tpl_module='" . addslashes($row['tpl_module']) . "',tpl_tplset='" . addslashes($tplset) . "',tpl_file='" . addslashes($tpl_file) . "',tpl_desc='" . addslashes($row['tpl_desc']) . "',tpl_type='" . addslashes($row['tpl_type']) . "'") ;
    ...
    while ($row = $db->fetchArray($result)) {
        ...
        if (! $db->getRowsNum($drs)) {
            ...
            while (list($tpl_id) = $db->fetchRow($drs)) {
                ...
                $db->query(
                    'UPDATE '
                    . $db->prefix('tplfile') . " SET tpl_refid='" . addslashes($row['tpl_refid']) . "',tpl_desc='" . addslashes($row['tpl_desc']) . "',tpl_lastmodified='" . addslashes($row['tpl_lastmodified']) . "',tpl_lastimported='" . addslashes($row['tpl_lastimported']) . "',tpl_type='" . addslashes($row['tpl_type']) . "' WHERE tpl_id='$tpl_id'") ;
...
function tplsadmin_copy_templates_f2db($tplset_to, $whr_append = '1')
{
    ...
    while ($row = $db->fetchArray($result)) {
        ...
        if (! $db->getRowsNum($drs)) {
            // INSERT mode
            $sql = 'INSERT INTO '
                   . $db->prefix('tplfile') . " SET tpl_refid='" . addslashes($row['tpl_refid']) . "',tpl_desc='" . addslashes($row['tpl_desc']) . "',tpl_lastmodified='" . addslashes($lastmodified) . "',tpl_type='" . addslashes($row['tpl_type']) . "',tpl_tplset='" . addslashes($tplset_to) . "',tpl_file='" . addslashes($row['tpl_file']) . "',tpl_module='" . addslashes($row['tpl_module']) . "'" ;
            $db->query($sql) ;
            $tpl_id = $db->getInsertId() ;
            $db->query('INSERT INTO ' . $db->prefix('tplsource') . " SET tpl_id='$tpl_id', tpl_source='" . addslashes($tpl_source) . "'") ;
            altsys_template_touch($tpl_id) ;
